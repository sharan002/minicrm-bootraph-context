...
const Header = () => {
  ...
  const tabCounts = {
    ...
    whatsapp: users.filter(
      (u) =>
        u.leadfrom?.toLowerCase() === "whatsapp" &&
        !u.respondedAfterFollowUp
    ).length,
    metaads: users.filter(
      (u) =>
        u.leadfrom?.toLowerCase() === "metaads" &&
        !u.respondedAfterFollowUp
    ).length,
    website: users.filter(
      (u) =>
        u.leadfrom?.toLowerCase() === "website" &&
        !u.respondedAfterFollowUp
    ).length,
    respondedAfterFollowUp: users.filter((u) => u.respondedAfterFollowUp).length,
    "manual entry": users.filter(
      (u) =>
        u.leadfrom?.toLowerCase() === "manual entry" &&
        !u.respondedAfterFollowUp
    ).length,
  };
        ...
        <div className="row align-items-center py-2">
          <div className="col-md-6 d-flex align-items-center">
            <div className="d-flex align-items-center me-4">
              ...
              <h1 className="h4 mb-0 fw-bold text-dark">Lead Manager</h1>
            </div>
            ...
            <div className="d-none d-md-flex">
              {["all", "whatsapp", "metaads", "website", "respondedAfterFollowUp", "manual entry"].map((tab) => (
                <button
                  ...
                  className={`btn btn-sm mx-1 ${activeTab === tab ? 'btn-primary' : 'btn-outline-secondary'}`}
                >
                  ...
                  <span className={`badge ${activeTab === tab ? 'bg-light text-dark' : 'bg-secondary'}`}>
                    {tabCounts[tab] || 0}
                  </span>
                </button>
              ))}
          ...
          <div className="col-md-6 d-flex align-items-center justify-content-end">
            ...
            <div className="dropdown me-2" ref={notificationRef}>
              <button 
                    ...
                    {unreadLeads.length > 0 ? (
                      unreadLeads.map(leadId => {
                        ...
                        return (
                          ...
                          >
                            <div className={`p-2 rounded-circle ${lead.respondedAfterFollowUp ? 'bg-success' : 'bg-primary'} me-3`}>
            ...
            <div className="d-flex overflow-auto pb-2">
              {["all", "whatsapp", "metaads", "website", "respondedAfterFollowUp", "manual entry"].map((tab) => (
                <button
                  ...
                  className={`btn btn-sm me-1 flex-shrink-0 ${activeTab === tab ? 'btn-primary' : 'btn-outline-secondary'}`}
                >
                  {tab === "respondedAfterFollowUp"
                    ? "Responded"
                    : tab === "manual entry"
                    ? "Manual"
                    : tab.charAt(0).toUpperCase() + tab.slice(1)}
                  <span className={`badge ms-1 ${activeTab === tab ? 'bg-light text-dark' : 'bg-secondary'}`}>
                    {tabCounts[tab] || 0}
                  </span>
                </button>
              ))}
